...
CMD java -jar -Dspring.profiles.active=${profile} -Djasypt.encryptor.password=${clave} framework-educativo-subject-0.0.1-SNAPSHOT.jar --eureka-host=${eureka_host} --subject-service=${subject_service} --subject-db=${subject_db} --spring.rabbitmq.host=${rabbit_host} --spring.rabbitmq.password=${rabbit_pass} --secret-token=${secret_token}